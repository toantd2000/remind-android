...
| `:features:reminder` | Feature | Chứa các màn hình CRUD (Danh sách, Chỉnh sửa, Báo chuông) và ViewModel. |
| `:core:reminder` | Infrastructure | **(Hạt nhân)** Quản lý Scheduler, Receiver và Service nhắc nhở. |
...
### A. Dependency Inversion qua `ReminderIntentProvider`
Tại sao? Do `:core:reminder` (Service) không được phép phụ thuộc vào `:app` (Activity), nhưng Service lại cần mở Activity khi nhắc nhở nổ.
- **Interface:** `ReminderIntentProvider` định nghĩa tại `:core:reminder`.
- **Implementation:** `ReminderIntentProviderImpl` thực thi tại `:app`, biết rõ `MainActivity`.
...
Luồng xử lý khi nhắc nhở nổ:
1. `ReminderReceiver` -> `ReminderService` (đổ chuông).
2. `ReminderService` -> `ReminderRingManager.setRinging(reminderId)`.
3. `MainActivity` (Root) quan sát StateFlow của `ReminderRingManager`.
...
- **Explicit Intent:** Tuyệt đối không dùng Implicit Intent (Action string) để gọi Service/Receiver nội bộ. Luôn qua `ReminderIntentProvider` hoặc gọi trực tiếp class trong cùng module.
