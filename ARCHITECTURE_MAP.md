...
Dự án Báo thức được xây dựng theo kiến trúc **Multi-module** kết hợp với **Clean Architecture**, nhằm tối đa hóa khả năng tái sử dụng, cô lập lỗi và quản lý sự phụ thuộc (Dependency Management).
...
| `:features:alarm` | Feature | Chứa các màn hình CRUD (Danh sách, Chỉnh sửa, Báo chuông) và ViewModel. |
| `:core:alarms` | Infrastructure | **(Hạt nhân)** Quản lý Scheduler, Receiver và Service báo thức. |
...
| `:core:designsystem` | Design System | Chứa các UI Components dùng chung (AlarmScaffold, Switch, Theme). |
...
Tại sao? Do `:core:alarms` (Service) không được phép phụ thuộc vào `:app` (Activity), nhưng Service lại cần mở Activity khi báo thức nổ.
- **Interface:** `AlarmIntentProvider` định nghĩa tại `:core:alarms`.
