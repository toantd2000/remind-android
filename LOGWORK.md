...
### [2026-05-07]
- **Bug Fix:** Tự động dọn dẹp trạng thái "Bỏ qua lần tới" khi mốc thời gian đó đã trôi qua.
- **Data Integrity:** Đảm bảo `skippedAt` trong Database được reset về null khi hết hạn để tránh sai lệch UI.
- **Automation:** Logic dọn dẹp được tích hợp ngầm vào `AlarmListViewModel` thông qua cơ chế phản ứng của Flow.
- **Testing:** Bổ sung Unit Test cho model `Alarm` để kiểm chứng logic hết hạn.
