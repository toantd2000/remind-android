...
### [TDR-034] - Tách biệt Logic Auto-Silence khỏi trạng thái Âm thanh (Audible State)
- **Ngày thực hiện:** 2026-04-22
- **Trạng thái:** Accepted
- **Bối cảnh:** Tính năng Auto-silence hoạt động không ổn định do timer bị gắn chặt với trạng thái âm thanh và bị reset khi người dùng vào màn hình nhiệm vụ. Ngoài ra, việc sử dụng `runBlocking` trong Controller gây block Main thread, dẫn đến giật lag và sai lệch thời gian xử lý.
- **Quyết định:** 
  - Tách biệt logic đếm ngược Auto-silence khỏi việc kiểm tra trạng thái Mute.
  - Chuyển đổi toàn bộ `ReminderController` sang `suspend` để xử lý bất đồng bộ chuẩn xác.
  - Sử dụng ID định danh rõ ràng (`reminderId`) khi gọi các hàm kết thúc phiên báo thức để tránh race condition khi có nhiều báo thức cùng lúc.
  - Đảm bảo mọi tác vụ DB đều chạy trên `Dispatchers.IO`.
- **Hệ quả:** Hệ thống hoạt động mượt mà, chính xác và không còn hiện tượng reset timer hoặc treo UI khi xử lý báo thức.
